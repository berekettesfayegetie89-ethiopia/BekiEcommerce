...
export function Stars({ rating, count, size = 'sm' }) {
  ...
  return (
    <div className="flex items-center gap-1">
      ...
      {count !== undefined && (
        <span className="link text-xs">({count.toLocaleString()})</span>
      )}
    </div>
  );
}
...
export default function ProductCard({ product }) {
  ...
  const price = product.isFlashSale &&
    product.flashSalePrice &&
    new Date(product.flashSaleEnds) > new Date()
  ...
  const handleAdd = async (e) => {
    ...
    try {
      ...
      // Show the real server message so you know exactly why it failed
      const msg = err.response?.data?.message || err.message || 'Failed to add to cart';
      // If the session expired the axios interceptor already logged the user out
  ...
  return (
    <div className="card-hover group relative flex flex-col h-full">
      ...
      <div className="absolute top-2 left-2 z-10 flex flex-col gap-1">
        ...
        {product.isFeatured && (
          <span className="badge bg-az-orange text-white">Featured</span>
        )}
      </div>
      ...
      <button
        ...
        className={`absolute top-2 right-2 z-10 w-8 h-8 rounded-full shadow-md flex items-center justify-center
          opacity-0 group-hover:opacity-100 transition-all hover:scale-110
          ${wishlisted ? 'bg-red-50 text-az-red' : 'bg-white dark:bg-dk-card text-az-muted hover:text-az-red'}`}
      >
      ...
      <div className="p-3 flex flex-col flex-1 bg-az-card dark:bg-dk-card rounded-b">
        <p className="text-az-muted dark:text-dk-sub text-[11px] uppercase tracking-wider mb-1">
          {product.brand}
        </p>
        <Link to={`/products/${product.id}`}>
          <h3 className="text-az-text dark:text-dk-text text-sm font-medium line-clamp-2
            hover:text-az-link dark:hover:text-dk-blue transition-colors mb-1 leading-snug">
            {product.name}
          </h3>
        </Link>
        ...
        <div className="mt-2 mb-3">
          <div className="flex items-baseline gap-2 flex-wrap">
            ...
            {discount > 0 && (
              <span className="text-az-red dark:text-dk-red text-xs font-bold">Save {discount}%</span>
            )}
          </div>
          {price >= 100 && <p className="link text-xs mt-0.5">FREE Shipping</p>}
          {product.stock === 0 && (
            <p className="text-az-red dark:text-dk-red text-xs font-bold mt-1">Out of Stock</p>
          )}
          {product.stock > 0 && product.stock <= 5 && (
            <p className="text-az-red dark:text-dk-red text-xs font-bold mt-1">
